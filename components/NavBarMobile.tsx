                ...
                <div className={isMenuOpen ? 'md:hidden  w-[50vw] h-screen transp bg-opacity-100 bg-[#0F4C75] transition-all duration-300 fixed z-10 rounded-r-xl' : ' w-0 h-0 transition-all duration-300'}> 
                    ...
                    <div    className="text-white translate-y-[50px] translate-x-[20px] w-[15px] space-y-2">
                        ...
                        <ul className={isMenuOpen ? 'md:hidden flex translate-y-[100px] flex-col gap-4 translate-x-6 xsm:text-[12px]' : 'hidden md:flex justify-end gap-4' }>
                            ...
                            <li><Link onClick={toggleMenu} className={window.location.pathname === "/HackTheBox" ? 'font-bold md:text-[24px] sm:text-[20px] xsm:text-[14px]':''} href={"/HackTheBox"} >HTB</Link></li>
                            <li><Link onClick={toggleMenu} className={window.location.pathname === "/Learn" ? 'font-bold md:text-[24px] sm:text-[20px] xsm:text-[14px]':''} href={"/ctf-writeups"} >CTF</Link></li>
                        </ul>
                    </div>
